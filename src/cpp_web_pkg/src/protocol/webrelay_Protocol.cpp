...
//------------------------------------------------------------------------
// Protocol: ReqFindInstance
//------------------------------------------------------------------------
void webrelay::s2c_Protocol::ReqFindInstance(netid targetId, bool isBinary, const string &name)
{
	cPacket packet(&s_packetHeader);
	packet.SetProtocolId( GetId() );
	packet.SetPacketId( 3724559230 );
	packet.SetPacketOption(0x01, (uint)isBinary);
	if (isBinary)
	{
		// marshaling binary
...
//------------------------------------------------------------------------
// Protocol: AckFindInstance
//------------------------------------------------------------------------
void webrelay::c2s_Protocol::AckFindInstance(netid targetId, bool isBinary, const string &name, const string &url, const int &result)
{
	cPacket packet(&s_packetHeader);
	packet.SetProtocolId( GetId() );
	packet.SetPacketId( 1628540119 );
	packet.SetPacketOption(0x01, (uint)isBinary);
	if (isBinary)
	{
		// marshaling binary
...
//------------------------------------------------------------------------
// Protocol: InstanceInfo
//------------------------------------------------------------------------
void webrelay::c2s_Protocol::InstanceInfo(netid targetId, bool isBinary, const string &name, const int &sessionCount)
{
	cPacket packet(&s_packetHeader);
	packet.SetProtocolId( GetId() );
	packet.SetPacketId( 2676095564 );
	packet.SetPacketOption(0x01, (uint)isBinary);
	if (isBinary)
	{
		// marshaling binary
		packet.Alignment4(); // set 4byte alignment
		marshalling::operator<<(packet, name);
		marshalling::operator<<(packet, sessionCount);
		packet.EndPack();
		GetNode()->Send(targetId, packet);
	}
	else
	{
		...
		try {
			put(props, "name", name);
			put(props, "sessionCount", sessionCount);
			stringstream ss;
			boost::property_tree::write_json(ss, props);
			packet << ss.str();
			packet.EndPack();
