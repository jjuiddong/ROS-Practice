...
public:
	...
	void ReqFindInstance(netid targetId, bool isBinary, const string &name);
...
public:
	...
	void AckFindInstance(netid targetId, bool isBinary, const string &name, const string &url, const int &result);
	void AckPing(netid targetId, bool isBinary, const int &id);
	void InstanceInfo(netid targetId, bool isBinary, const string &name, const int &sessionCount);
	static cPacketHeaderJson s_packetHeader;
};
}
